...
def test_parameter_report_single_path():
    # round44：运行文件不再直接引用维护区路径，统一指向 maintenance-map 集中登记
    assert "references/parameter-build-report.md" not in CORPUS
    assert "维护区参数构建报告" in CORPUS


def test_breakfast_mode_three_states():
    for kw in ("breakfast_mode", "planned", "guidance_only", "excluded", "默认不列具体采购"):
...
def _minimal_plan():
    return {
        "plan": {"date_range": "2026-08-23 至 2026-08-29", "subtitle": "平衡激素", "stats": {"天数": "7 天", "模式": "平衡激素", "进食窗口": "07:30–19:30", "预算": "约 ¥231"}},
