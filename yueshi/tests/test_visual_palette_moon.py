...
# 2) 月相算法：2026-08-28 为满月（round39 收窄满月窗口）；无满月周不得出现"满月"
sys.path.insert(0, os.path.join(ROOT, "scripts"))
import importlib.util
spec = importlib.util.spec_from_file_location("rp", os.path.join(ROOT, "scripts", "render_plan.py"))
rp = importlib.util.module_from_spec(spec); spec.loader.exec_module(rp)
illum, waxing, name = rp.moon_phase(date(2026, 8, 28))
assert name == "满月" and illum > 0.95, (illum, name)
illum1, _, name1 = rp.moon_phase(date(2026, 8, 26))
assert name1 in ("盈凸月", "满月") and illum1 > 0.9, (illum1, name1)
