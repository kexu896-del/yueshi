...
- 外部抓价/Provider 问题 → 修改 `references/price-provider-policy.md` 或对应 validator/router；
- 所有修改写入 `CHANGELOG.md`。
...
references/price-provider-policy.md  外部价格 Provider 契约（边界/路由/时机/状态映射/P01–P09/快照元数据/性能边界）
...
data/functional-medicine-rules.json  功能医学五书 A 级规则库（84 条，round70.1；auto_tip 供执行提示选择，数值仅参考、不进生产参数）
...
scripts/functional_medicine_rules.py  功能医学规则库加载/校验/执行提示选择（CLI：--mode/--context/--limit/--validate；协议与安全类永不自动）
...
scripts/price_result_validator.py  price-result.json P01–P08 校验 + price_result_hash；P09 一致性门禁在价格合并/渲染侧执行（契约 references/price-provider-policy.md）
scripts/price_provider_router.py   Provider 状态→门禁映射 / price_basis 赋值 / price_snapshot_meta 构建（纯函数，不启动浏览器）
schemas/price-query.schema.json    外部抓价输入契约（price_data_version = 1.0）
schemas/price-result.schema.json   外部抓价结果契约（与 yueshi-dingdong-helper 模型一一对应）
...
book-extraction/how_to_eat_cookbook/  How to Eat to Beat Disease Cookbook 候选 + 5×5×5 防御食物清单（附录 A 205 项）
book-extraction/the-disease-delusion/  The Disease Delusion 功能医学精读提炼报告（round70）
book-extraction/good-energy/        Good Energy 精读提炼报告（round70，稳糖规则集成度最高）
book-extraction/why-we-get-sick/    Why We Get Sick 精读提炼报告（round70，胰岛素抵抗框架）
book-extraction/beat-autoimmune/    Beat Autoimmune 精读提炼报告（round70，自免/肠道修复参考）
book-extraction/deep-nutrition/     Deep Nutrition 精读提炼报告（round70，传统饮食四支柱）
...
- `data/nutrition-adjustment-options.json` 统一营养调整候选表（由 build_adjustment_options.py 从 foods-table 生成，禁止手填 nutrition_delta）
- `schemas/nutrition-adjustment-options.schema.json` 候选表 JSON Schema
- `scripts/build_adjustment_options.py` 候选表生成器
- `scripts/nutrition_adjuster.py` 统一营养调整器（差额向量 / 来源受控 / 就地更新 meal.ingredients / 最多两轮）
- `tests/test_nutrition_adjustment_options.py` / `test_adjustment_source_scope.py` / `test_adjustment_updates_meal_ingredients.py` / `test_two_pass_limit.py`

## round39 新增文件

- `data/golden/golden-sample-plan.json` 人工确认的基准计划（golden sample，每次改版比对）
- `tests/test_round39_consistency.py` / `tests/test_golden_sample.py`

## round40 新增文件（家庭版 v0.3 RC2）

- `schemas/household/`（5 份：shared-definitions / base / overrides / effective / bundle）
- `scripts/household_reference_validator.py` 跨引用校验器（RC2-3）
- `scripts/build_household_golden_cases.py` + `data/golden/household/`（8 组案例，RC2-5）
- `tests/test_household_rc2.py` / `tests/test_household_skill_routing.py`

## 发布与维护区策略（round44 定稿）

- **维护区**：`book-extraction/` 为维护期书籍提取与审计工作目录，可能包含版权原文或中间提取材料，**不属于技能运行依赖**，不进入 yueshi.skill、yueshi.zip 或其他公开分发产物；只存在于受控源码或维护环境。维护期路径（供核对，运行文件不得引用）：`book-extraction/nutrition-audit/parameter-build-report.md`、`book-extraction/proposed-new-recipes.jsonl`、`book-extraction/duplicate-merge-plan.json`、`book-extraction/<book>/extraction/*`。
- **文件分类**：repo-only = book-extraction/、candidates/、__pycache__、scripts/extract_*（维护期书籍提取工具）；release-only = 无（全部运行文件随包发布）；`data/library-manifest.json` 中的维护区路径是**来源隔离登记表**（登记候选不直接入池），属元数据而非运行读取，保留在包内。
- **四条硬门禁**：① 运行脚本、规则文件（references/ workflows/）与 Schema 不得引用 book-extraction/（data/*.json 的来源登记除外）；② 发布包不得包含该目录内容、原始电子书文件（.epub/.mobi/.azw/.azw3）、提取缓存或大段原文副本——release_pack 包后载荷扫描命中即失败；③ 书籍提炼的运行规则必须经结构化、来源标记和审核，不得以原文附件入包；④ 依赖闭包发现运行文件引用被排除目录 → 构建立刻失败。
- **唯一发布入口**：`scripts/release_pack.py`；正式发布不得绕过它手工压包；构建可复现（固定排序/时间戳/权限位，重复构建 SHA-256 一致）。
