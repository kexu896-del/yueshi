...
from render_plan import (REQUIRED_PLAN_KEYS, CATEGORY_ORDER, _display_price,  # noqa: E402
                         ...
                         _normalize_provider_name, check_provider_name_leak)
...
def _md(plan):
    ...
    # round54/55：与 HTML/PDF 同一 7 列口径（食材/需要量/建议购买规格/参考价/购买备注/替代/剩余去向），
    # 叮咚商品名以「（叮咚：…）」附在标准名后（Markdown 无双行样式，同行小注）。
    out += ["| 品类 | 食材 | 需要量 | 建议购买规格 | 参考价 | 购买备注 | 替代 | 剩余去向 |",
            "|---|---|---|---|---|---|---|---|"]
    for cat in order:
        for i in groups[cat]:
            subs = "、".join(i.get("substitutes", [])) or "—"
            name = i['ingredient']
            ppn = (i.get("provider_product_name") or "").strip()
            if ppn:
                name += f"（叮咚：{_normalize_provider_name(ppn)}）"
            out.append(f"| {cat} | {name} | {i.get('required_quantity','')} | "
                       f"{i.get('acceptable_package','')} | {_display_price(i)} | "
                       f"{i.get('purchase_note','')} | {subs} | {i.get('leftover_action','')} |")
    if shopping.get("total"):
        out += ["", f"**本周预计支出：{shopping['total']}**"]
    if shopping.get("price_note"):
        out += ["", shopping["price_note"]]
    # round56 格式差异化：Markdown 默认完整保留「对应菜单及用量」，
    # 按食材分组列表（避免宽表格）；不含 provider_product_name。
    alloc = [i for i in items if i.get("meal_allocation")]
    if alloc:
        out += ["", "## 食材对应菜单及用量", ""]
        for i in alloc:
            out.append(f"### {i['ingredient']}，共 {i.get('required_quantity','')}")
            for seg in re.split(r"[、;；]+", i["meal_allocation"]):
                if seg.strip():
...
def main():
    ...
    check_provider_name_leak(plan)  # G12 旁路：商品名泄漏门禁对全部格式生效
    md = _md(plan)
