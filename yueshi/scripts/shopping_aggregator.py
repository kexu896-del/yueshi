...
STOCK_CATEGORY = {
    ...
    # 2026-09-18：菜单 CSV 直接使用展示分区名（render_plan CATEGORY_ORDER），
    # 此前未登记导致乳品落回"可选点缀→蔬菜"误报（核心食材 18>16）。
    "蔬菜与菌菇": "vegetable", "肉蛋水产与豆制品": "protein",
    "谷物薯类与主食": "staple", "乳品与油脂": "oil_nut", "乳品": "oil_nut",
}
...
def main():
    ...
    ap.add_argument("--budget-type", choices=["hard_cap", "preferred_target", "flexible_target"],
                    default=None,
                    help="预算类型（round65）：hard_cap 硬上限（超出需用户批准否则阻止定稿）/"
                         "preferred_target 优先目标（默认 allowed_overrun_pct=5%）/"
                         "flexible_target 弹性参考（默认 allowed_overrun_pct=15%）")
    ap.add_argument("--allowed-overrun-pct", type=float, default=None,
                    help="允许超出比例（%%）；缺省按 budget_type 默认（hard_cap=0, preferred=5, flexible=15）")
    ...
    # round65：菜单 CSV 缺 category 或 category 不在 STOCK_CATEGORY 时，
    # 用 data/ingredient-catalog.json 的 category 字段回填分区（不再按展示名猜测，
    # 避免"共 N 种必买"却报"核心食材 0 种"的误报）。
    _cat_fallback = {}
    _catalog_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 "..", "data", "ingredient-catalog.json")
    if os.path.exists(_catalog_path):
        _cat_items = json.load(open(_catalog_path, encoding="utf-8")).get("items", {})
        _CAT_TO_ZONE = {"蔬菜": "vegetable", "蛋白质": "protein", "天然碳水": "staple",
                        "味型": "oil_nut", "调味": "oil_nut", "可选点缀": "vegetable"}
        # 2026-09-18："可选点缀"是混合类（乳品/坚果/水果/油/菜），按 ingredient_family 细分，
        # 乳品与油脂不得计入蔬菜区（对齐 CSV 分类口径）。
        _FAMILY_TO_ZONE = {"dairy": "oil_nut", "oil": "oil_nut", "nut": "oil_nut",
                           "flavor": "oil_nut", "other": "oil_nut", "fruit": "staple",
                           "crucifer": "vegetable"}
        for _name, _it in _cat_items.items():
            _zone = _CAT_TO_ZONE.get(_it.get("category", ""))
            if _it.get("category") == "可选点缀" and _it.get("ingredient_family") in _FAMILY_TO_ZONE:
                _zone = _FAMILY_TO_ZONE[_it["ingredient_family"]]
            if _zone:
                _cat_fallback[_name] = _zone
                for _al in _it.get("aliases", []):
                    _cat_fallback.setdefault(_al, _zone)
    ...
    for r in rows:
        ...
        foods[food]["zone"] = STOCK_CATEGORY.get(
            r.get("category", ""),
            _cat_fallback.get(food, foods[food]["zone"]))
    ...
    if args.meals_per_day:
        ...
        if core == 0 and foods:
            print("  ! 分区缺失：全部食材落入调味区，已按 ingredient-catalog.json category 回填；"
                  "若仍为 0 请检查菜单 CSV 的 category 列")
    ...
    # 预算结构（round65：预算三态；hard_cap 超出且未经用户批准 → 阻止定稿）
    if args.budget:
        budget_type = args.budget_type or "preferred_target"
        default_overrun = {"hard_cap": 0.0, "preferred_target": 5.0, "flexible_target": 15.0}
        overrun_pct = (args.allowed_overrun_pct if args.allowed_overrun_pct is not None
                       else default_overrun[budget_type])
        type_label = {"hard_cap": "硬上限", "preferred_target": "优先目标",
                      "flexible_target": "弹性参考"}[budget_type]
        print("\n【预算结构】")
        print(f"  预算金额: ¥{args.budget:g}  类型: {type_label}（{budget_type}）"
              f"  允许超出: {overrun_pct:g}%  周期: {args.budget_period or '未指定'}")
        est_total = 0.0
        unpriced = []
        for f, d in foods.items():
        ...
        if not prices:
            ...
            over_limit = est_total > args.budget * (1 + overrun_pct / 100.0)
            over_target = est_total > args.budget
            print(f"  状态: {'超出允许范围' if over_limit else ('超出目标但在允许范围内' if over_target else '预算内')}")
            if over_target:
                print("  建议：替换高价等价食材（三文鱼→鲭鱼、牛里脊→鸡腿/蛋/豆腐、藜麦→糙米等），不得削减蛋白/热量/蔬菜/过敏原底线。")
            if budget_type == "hard_cap" and over_target:
                print("  ✗ 硬上限超支：未经用户明确批准不得定稿；先压缩方案或请用户确认提高预算。")
                sys.exit(2)
    ...
    for c in checks:
        print(" ", c)
    # 2026-09-18：包装单位口径告警——包装规格用枚/根/个等计件单位、需求量按克计时，
    # 预算按整包装计费，若单价表按克/份混填会放大金额（试跑曾出现 10枚×50 的倍率误算）。
    unit_warn = [f for f, d in foods.items()
                 if f in packages and packages[f][1] not in ("g", "kg", "ml", "l", "")]
    if unit_warn:
        print(f"  ! 包装单位与克重口径不一致（按整包装计费，预算请人工核对）：{'、'.join(unit_warn)}")
