...
- **plan.json 版本门禁（plan_meta 契约）**：plan.json 必须携带 `plan_meta` 版本块，字段固定为 `build_id` / `plan_schema_version` / `rules_bundle_hash` / `skill_version` / `runtime_rules_version` / `nutrition_rules_version` / `output_policy_version` / `recipe_manifest_version` / `effective_parameters_version` / `price_data_version`。渲染前校验：① plan_schema_version 与渲染器支持版本一致；② rules_bundle_hash 与当前规则文件组合哈希一致（存在时强制比对；plan_mode ∈ household_plan_modes 时哈希输入追加 references/household-runtime-rules.md、workflows/household-planning-flow.md 与 schemas/household/ 契约清单）；③ effective_parameters_version 与计算日志一致；④ recipe_manifest_version 与 ranker 日志一致。任一不满足即停止渲染，防止新版生成器配旧版渲染器、新版营养参数配旧版 plan.json 混跑。四个版本/哈希字段职责唯一：rules_bundle_hash = 规则匹配；pipeline_bundle_hash = 执行组件匹配；price_result_hash = 抓价结果完整性（不属 plan_meta 固定字段，记录在 price_snapshot_meta.result_hash）；price_data_version = 价格数据契约版本（price-query / price-result Schema 契约版本，当前 "1.2"）。使用外部价格 Provider 时 plan.json 追加可选顶层块 `price_snapshot_meta`（字段与条件见 references/price-provider-policy.md §6；契约 1.2 起为 delivery_context_confirmed_by_user 配送会话确认，不再含地址字段）。
- **采购门禁 P01–P09**：外部价格 Provider 接入后，采购与价格完整性按 P01（合法 price_basis）/ P02（direct_public_price 必需字段）/ P03（包装候选规格可解析）/ P04（价格单位显式确认）/ P05（普通价/活动价/会员价不混写）/ P06（售罄不作正式候选）/ P07（相关性与排除词）/ P08（request_id、Schema 与哈希匹配）/ P09（结果根级 provider / schema_version / request_id 与 price_snapshot_meta 完全一致，result_hash 与 price_result_hash 同源，不一致即拒绝渲染、不自动择值）逐项检查；各门禁出错处理（降级 / 换候选 / 拒绝候选 / 拒绝结果文件但不判菜单损坏）以 references/price-provider-policy.md §7 为唯一定义，执行组件为 scripts/price_result_validator.py。G06 自动修复范围限定见 price-provider-policy.md §10。
...
- **组件黑名单（永久禁用）**：目录页/TOC 组件（一周文档不需要目录）、环形图/饼图/甜甜圈图等一切营养图形组件（营养信息以每日一行合计呈现）。**食材↔菜谱对照（menu-map）round56 起改为格式差异化展示**：plan.json 始终保留完整映射数据（`meal_allocation` 或 `shopping_meal_usage_map`），显示与否由各格式渲染策略决定——HTML 默认完整保留（可折叠展开）、Word 默认保留（建议附录、跨页重复表头）、Markdown 默认保留（按食材分组列表）、PDF 固定不展示（hidden 为固定规则而非用户选项，配置固定为 `{html: full, docx: full, markdown: full, pdf: hidden}`，详见下文「对应菜单及用量的格式差异化策略」）；任何格式不得显示"其余若干项见网页版展开"。
...
- **页数硬约束与压缩顺序**：一周计划 **PDF ≤9 页（含封面）**；家庭版页数预算为基础 9 页 + 成员附页（摘要页最多并排 3 名成员，超出成员进附页），家庭版禁止为卡页数跌破字号下限。字号下限：正文 ≥9pt、采购表格 ≥8.5pt、行高 ≥1.25——不得为压页数跌破下限。新增内容导致超页时按固定顺序压缩：①删重复说明 → ②合并备注列 → ③简化平替与余量表述 → ④调边距 → ⑤在下限内轻微缩字号 → ⑥仍不行才允许第 10 页并在内部日志记录原因。免责声明与末节同页。
...
- **食养之理与执行说明（合并节，必保留）**：理论部分为 3 段自然叙述、不列点，每段 2 至 3 句、每句一个信息点——第一段：周期阶段 + 禁食安排 + 碳水选择逻辑；第二段：时令食材 + 蛋白质搭配 + 体质适配（如缺铁/胀气）；第三段：简要操作提示。执行提示 4 至 6 条、每条 15 至 25 字（用药间隔/外食策略/备餐顺序/不适调整），不重复正文已解释的内容，不出现推动互动的句子；对应 plan.json 的 `wisdom.paragraphs` 与 `execution_tips` 字段。其中 0 至 2 条可来自 `data/functional-medicine-rules.json` 的 auto_tip 规则（按模式选择、tip_key 去重，口径见 runtime-rules.md「功能医学行为规则」）。节内只允许"本周怎么安排 / 本周执行要点"两个固定子标题（同字号加粗，见"食养节子区块"）；全部达标时不重复罗列营养数字。
- **非做饭日标准文案（round45）**：excluded 日统一写"今天不在家用餐，本计划不安排菜单，也不计入本周采购与营养合计"；禁止"无人在家吃饭，三餐自行解决"一类表述（渲染门禁硬校验）。
...
及以上字段在最终用户文件（HTML/PDF/MD/DOCX）中对应的可见文字。禁用词：**周期协议**、source_verified、audit_status、source_rule_id、protocol_version、effective_parameters、safety_policy、reason_code、accepted_batch、locked_basket、provisional_locked_basket、plan.json、gate_result、low confidence、runtime-rules、nutrition-routing；**round57 扩充（Round55/56 新增内部状态与字段）**：awaiting_price_result、price_result_received、opt_out_after_query、dingdong_helper_opt_in、menu_snapshot_hash、nutrition_snapshot_hash、shopping_demand_hash、price_result_hash、migration_log、provider_product_name、shopping_meal_usage_map。内部字段无需改名；显示转换：周期协议 → 阶段适配；协议参数 → 本周饮食安排；协议执行 → 按当前阶段安排；reason_code → 未入选原因；accepted_batch → 已确认食材；locked_basket → 本周食材；low confidence → 区间估算；**round57 新增转换**：awaiting_price_result → 等待叮咚价格结果；price_result_received → 已读取价格结果；opt_out_after_query → 已改用参考价格估算；migration_log → 文件兼容处理记录。**round59 新增价格失败用户话术**：单个食材叮咚查价失败并降级估算时，用户可见说明统一写作「叮咚没查到价，已按当地参考价估算」（不得出现 search_empty / candidates_filtered / no_match 等内部状态词）。这些词允许出现在 plan.json、日志、Schema 和开发文档中，只禁止进入最终用户文件的可见文字。渲染器 validate 强制检查。

**round67 扩充（用户表达层内部词）**：用户可见文字不得出现——新版试跑、试跑、江淮时令覆盖层、覆盖层、蛋白角色对齐、角色对齐、热量补偿、自动追加、主餐口径、不计入碳水上限、不计上限、降权保留、评分过程、构建状态、纠错记录、校验通过、门禁结果、审核语言。以上词只允许存在于 plan.json 内部字段、日志、审计与开发文档；渲染器对可见文本区强制检查。

## 用户表达层（round67，2026-09-18）

正式用户文件中的每段文字必须至少回答一个问题：本周要做什么 / 为什么这样安排 / 哪些可以灵活调整 / 需要注意什么 / 预计花多少钱。不能回答以上任一问题的构建、调试、校验与评分信息不进入正式用户文件。

- **消息类型**：可见 = action / explanation / flexibility / caution / price_summary；内部 = build_status / algorithm_reason / validation_result / correction_trace / scoring_trace / gate_summary / debug_summary（只保留在 plan.json、日志、审计报告、价格结果与调试日志）。
- **模式展示**：显示名称取自 `data/mode-display.json`（hormone_balance → 平衡激素模式；keto_biologic → 酮生物模式），并附一句通俗执行说明；内部枚举、mode_schedule_hash 与推算/校验字段不得进入用户文件；模式仍由系统推算，不作为问卷选项。
- **可选加餐**：统一写"可选加餐"，说明按饥饿程度决定（如"下午明显饿时再吃；午餐吃得足，可以不吃"）；用户文件不得出现"热量补偿 / 自动追加 / 补偿加餐不计入碳水上限"。
- **每日营养**：PDF 默认不逐日展示计算口径——有结构化 `days[].nutrition_summary` 时用其中 `user_line`（如"今日营养：整体符合本周安排。"）；HTML/Markdown 可展示参考估算行（如"参考估算：约 1565 kcal，蛋白质约 102g。食堂午餐按常见份量估算，实际会有浮动。"）；PDF 不得出现"主餐口径 / 不计入上限 / 蛋白偏高提示 / 活动量低时去掉加餐鸡蛋"。
- **预算摘要**：目标预算与允许范围分开说明（"本周预计采购约 184 元。预算目标为 150 元，可接受范围约至 158 元，目前比可接受范围高约 26 元。"）；节省建议最多两条，使用"预计节省约 X 元"；不得出现"压回线内 / 优先目标 +5% 线"。
- **估价说明**：参考估价说明在 PDF 中只出现一次（"酸奶、全麦面包和豆腐暂按当地常见价格估算，购买时以实际价格为准。"）；采购表单行只写"当地参考价"，不得在采购表、预算总结和执行要点重复解释查价失败。
- **最后一页**：统一标题"本周执行提醒"，最多 5 条，不重复封面、本周阶段、采购表和七天菜单的整段信息（PDF 渲染超过 5 条即失败）。
- **转换函数**：统一由 `scripts/common/visible_messages.py` 生成用户文案（模式、加餐、蛋白、预算、估价、时令、查价失败话术）。
...
- **封面摘要卡四卡固定（round58 恢复）**：`plan.stats` 固定 天数｜模式｜进食窗口｜预算 四张等宽卡；阶段变化移入副标题摘要行；模式卡主值最多一行，比例放标签辅助行；预算卡的"叮咚页面价"口径说明放标签或封面摘要；家庭模式同样四卡；详细排版令牌见 visual-spec.md「首页 cover 强制视觉」。
- **分页修复不得改业务文本（round58）**：采购表分页只通过样式与分页规则解决（整行 break-inside: avoid、分类标题与首行绑定、表头跨页重复、剩余空间不足整行移下一页）；**禁止为消除孤行/跨页而缩短或改写食材名、菜名、需求量、剩余去向等业务文本**；渲染失败只调整样式或分页后重跑渲染层。

- **当日建议贴卡**：`.day-advice` 必须与所属日卡同页（break-before/break-inside: avoid），不得作为独立色条漂到下一章节上方。
- **menu-map（round56 格式差异化）**：「对应菜单及用量」按格式差异化展示，见下文专节；无截断行、无"见网页版展开"提示。
...
## 两阶段渲染门禁与采购表排版（round54 新增，round55 定稿 G12/G13）

- **G12 等待价格结果门禁（入口级最终门禁，user_input_required，非 fatal）**：`dingdong_price_choice = use_helper`（派生字段 `dingdong_helper_opt_in = yes`，只读）且 `price_workflow_state = awaiting_price_result`（正式查价清单已交付、结果未回传）时禁止生成 PDF、Word、最终食谱或最终采购清单——只交付「月食-查价清单.json」与操作提示。渲染总控 `render_output.py` 在入口检查该状态并拒绝全部正式格式（报 G12 / price_result_pending）；拒绝渲染是正常等待状态，不属于 fatal。收到合规价格结果后状态转 price_result_received；用户明确放弃转 opt_out_after_query（允许估价生成，必须注明估价口径）。
- **G13 价格结果格式门禁（入口级，user_input_required）**：结果根级 schema_version / provider / request_id 必须齐备且 request_id 与本次查价清单一致；分支处理与迁移约束以 price-provider-policy.md §4.2 为唯一定义。任何内容完整性判断都不能绕过 Schema。
- **执行门禁 M11（round60：模式一致性，fatal）**：`basket_builder` / `recipe_ranker` / 营养求解等所有下游脚本使用的模式摘要必须与已冻结 `mode_schedule_hash` 一致；任一脚本的 keto_days / hormone_days / mode 参数与冻结计划不一致时，**流程失败关闭（fatal: mode_schedule_mismatch），不得继续生成菜单**。下游脚本一律以 `--mode-schedule` 读取唯一已冻结文件，禁止手写独立模式参数（语义见 runtime-rules.md「模式参数唯一来源」）。
- **渲染门禁 M01–M10**：M01 plan.json 校验通过；M02 rules_bundle_hash 匹配；M03 价格行无空价；M04 provider_product_name 未泄漏至菜单/菜谱/餐次/做法/复热说明/营养说明（仅允许出现在 shopping_items 与 price_snapshot；menu-map 模块同样不得读取商品名）；M05 采购表为 7 列结构且购买备注列存在；M06 menu-map 按格式差异化策略渲染（HTML/Word/Markdown=full，PDF 固定 hidden 无残留；任何格式不得出现"见网页版展开"）；M07 字号不低于下限；M08 页数 ≤ 预算；M09 免责声明非独占页；M10 等价于 G12 渲染侧执行（awaiting_price_result 状态下无渲染输出）。任一失败即停止渲染并报告门禁编号。

### 对应菜单及用量的格式差异化策略（round56 定稿）

- **数据层**：`meal_allocation` / `shopping_meal_usage_map` 必须完整保存在 plan.json——用于校验采购总需求与逐餐用量、校验食材是否进入菜单、生成剩余去向、支持 HTML/Word/Markdown 展示；PDF 不展示不等于删除业务数据。
- **HTML**：完整保留，默认折叠、可展开；摘要显示食材/总需求量/使用餐次数，展开后显示日期、餐次、菜名和用量。
- **Word**：完整保留，默认置于附录（"附录：食材对应菜单及用量"），允许跨页并重复表头。
- **Markdown**：完整保留，按食材分组展示（食材标题 + 逐餐项目符号），不使用超宽表格。
- **PDF：固定 hidden，不渲染该模块**——不出现章节标题、表头、残留行或空白占位页；**不再支持 auto/full/summary 试排、版面评估、模式降级与映射表评估日志**（完整展开约占 3 页、与七天菜单重复，并增加跨页/字号压缩/采购表拥挤风险）。显式把 pdf 配成非 hidden 即渲染失败。
- **配置**：`output_options.meal_usage_map` 固定为 `{html: full, docx: full, markdown: full, pdf: hidden}`。
- **通用约束**：所有格式均只读取标准食材名，`provider_product_name` 不得进入该模块；不得显示"其余项目见网页版展开"；映射模块的显示策略不得影响采购总量和菜单用量的一致性校验。
- **采购表排版规范（round54/55）**：采购清单固定 7 列——食材｜需要量｜建议购买规格｜参考价｜购买备注｜替代｜剩余去向。食材列双行显示：主行为标准食材名，次行（浅灰色、比正文小约 1pt）为叮咚实际商品名 `provider_product_name`（格式"（叮咚：…）"；商品原名中的嵌套括号先转换为逗号或中点，仅当有查价结果时显示）；**provider_product_name 只允许出现在采购清单与 price_snapshot**，七天菜单、菜名、逐餐用量、做法、复热说明中出现即渲染失败（render_plan.py `check_provider_name_leak` 字段级+值级硬门禁）。预切、净菜、附独立调味包等信息写入「购买备注」列；**购买备注与剩余去向分开**，"附调味包可不用"不得写入剩余去向。表格行高自动撑开、禁止固定高度；长名称溢出优先换行与调整列宽，不继续压缩字号；无备注时留空不占行高。

## 文案与数据同源（round39 新增，系统性防脱钩）

- **一切日期、克数、价格、星期文案必须从 plan.json 字段派生**：渲染器与模型在最终阶段不得新增任何手写静态文本（如"周二 8/25 晚""400g""约105元"）；备餐日/采购日标签中的"周X M/D"由渲染器对照真实星期校验，不符即渲染失败。
- **采购清单"一行一食材"硬门禁**：一个采购行只含一种食材，禁止"排骨+玉米"一类合并行；渲染前 lint 与 validate 双层检查。
- **预算双口径**：库存（from_inventory）食材价格列显示"库存（参考 X元/规格）"；预算区同时展示"本周实际支出（不含库存）"与"含库存全口径约"，防止真实开支被库存隐形。
- **月相只作视觉记录且必须准确**：月相轨道使用连续相位算法，满月/新月窗口收窄至各约 ±0.8 天；一周轨道不得出现 3 天及以上同名"满月"。
- **功效断言门禁**：可见文字不得出现未经来源层级标注的功效表述（如"对骨骼/血脂/代谢更友好""延缓衰老""治疗"），validate 正则扫描。
...
plan_meta 在明细版本字段之外增加：`build_id`（如 yueshi-2026.09-v1）与 `rules_bundle_hash`（runtime-rules + nutrition-routing + output-policy + safety-rules + effective-parameters + recipe-manifest 的组合哈希；家庭模式追加 household-runtime-rules + household-planning-flow + schemas/household 契约清单）。渲染器优先校验 plan_schema_version 与 rules_bundle_hash；各明细版本字段用于日志和排错。
