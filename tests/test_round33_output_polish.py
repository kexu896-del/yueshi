...
def test_menu_map_print_compression():
    assert "menu-map-extra" in RP and "menu-map-print-note" in RP
    assert ".menu-map-extra { display: none; }" in PRINT
    ...
    rc, out = _render(plan)
    assert rc == 0 and out.count("<tr class=menu-map-extra>") == 4 and "其余 4 项食材映射见网页版展开" in out
